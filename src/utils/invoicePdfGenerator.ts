...
export function generateInvoicePdfBuffer(invoice: InvoiceForPdf): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const blockBottom = Math.max(y + 110, jy + 4);
    y = blockBottom;
    ...
    const drawTableHead = (yy: number) => {
      ...
      cols.forEach((c, i) => {
        doc.text(c.label, colX[i] + 4, yy + 6, {
    ...
    const _bankBlockH = (_showPayment || _showBank)
      ? (12 + Math.max(_showPayment ? 12 : 0, _bankRowsForHeight.length * 11) + 4)
      : 0;
    const _aedHere = toAed(invoice.total, invoice.currency) !== null;
    const _totalsRowsH = 16 + 16 + (invoice.shippingCost > 0 ? 16 : 0);
    const _aedBlockH = _aedHere ? 36 : 0;
    const _amountWordsH = invoice.amountInWords ? 18 : 12;
    const FOOTER_TOTAL_H =
      8 +                                          // divider + gap
      _totalsRowsH + 4 + 24 + _aedBlockH + 4 +      // totals + TOTAL band + AED block
      _amountWordsH +                              // words box
      8 +                                          // divider + gap
